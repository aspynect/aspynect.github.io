/*:
* @plugindesc Sends information to LiveSplit to enable autosplitters/load removal (with debug logging prompts)
...
(function() {
    ...
    // --- Debug Prompt + Logging Helpers ---
    function promptEventName(defaultName) {
        var rawName = window.prompt("Name this trigger:\n" + defaultName, "");
        return (rawName && rawName.trim().length > 0) ? rawName.trim() : defaultName;
    }

    function logEvent(defaultName) {
        if (!DEBUG_EVENT_LOGGING) return;
        var eventName = promptEventName(defaultName);
        console.log("Triggered: " + eventName);
        try {
            fs.appendFileSync("EventLog.txt", eventName + "\n");
        } catch (e) {
    ...
    Game_Switches.prototype.setValue = function(switchId, value) {
        if (!!value != !!$gameSwitches.value(switchId)){
            var defaultName = `switch${switchId} -> ${!!value}`;
            logEvent(defaultName);
    ...
    Game_Variables.prototype.setValue = function(variableId, value) {
        if (value != $gameVariables.value(variableId)){
            var defaultName = `variable${variableId} -> ${value}`;
            logEvent(defaultName);
    ...
    Game_Map.prototype.setupStartingMapEvent = function() {
        ...
        for (var i = 0; i < events.length; i++) {
            var event = events[i];
            if (event.isStarting()) {
                var defaultName = `map${this.mapId()}_event${event._eventId}_page${event._pageIndex}`;
                logEvent(defaultName);
    ...
    Game_Interpreter.prototype.command117 = function() {
        ...
        logEvent("commonEvent_call:" + this._ls_nextid);
    ...
    Game_Interpreter.prototype.setupChild = function(list, eventId) {
        ...
        logEvent("commonEvent_child:" + this._ls_nextid);
    ...
    Game_Map.prototype.setupAutorunCommonEvent = function() {
        for (var i = 0; i < $dataCommonEvents.length; i++) {
            ...
            if (event && event.trigger === 1 && $gameSwitches.value(event.switchId)) {
                logEvent("commonEvent_autorun:" + event.id);
    ...
    Game_CommonEvent.prototype.update = function() {
        if (this._interpreter && !this._interpreter.isRunning()){
            logEvent("commonEvent_parallel:" + this._commonEventId);
    ...
    Game_Interpreter.prototype.executeCommand = function() {
        if (this._ls_splits){
            ...
            if (this._ls_splits.length > 0 && 
               (this._index == this._ls_splits[lasti].line || 
               (this._index >= this._list.length && this._ls_splits[lasti].line == -1))){
                
                var defaultName = `split_trigger: map${this._mapId}_event${this._eventId}_line${this._index}`;
                logEvent(defaultName);
